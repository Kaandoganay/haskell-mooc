...
longest [] = []
longest [xs] = xs
longest (xs:ys:xss) | length xs < length ys = longest (ys:xss)
                    | length xs > length ys = longest (xs:xss)
                    | length xs == length ys = if take 1 xs <= take 1 ys then  longest (xs:xss) else longest (ys:xss)
...
incrementKey x = map (increment x)
      where increment x (a,b) = if x == a then (a,b+1) else (a,b)
...
winner scores player1 player2 | Map.findWithDefault 0 player1 scores >= Map.findWithDefault 0 player2 scores = player1
                              | otherwise = player2
