...
member x (Set xs) = if x `elem` xs then True else False
...
instance Operation2 Add2 where
  compute2 (Add2 x y) = x + y
  show2 (Add2 x y) = show1 (Add1 x y)

instance Operation2 Subtract2 where
  compute2 (Subtract2 x y) = x - y
  show2 (Subtract2 x y) = show1 (Subtract1 x y)

instance Operation2 Multiply2 where
  compute2 (Multiply2 x y) = x * y
  show2 (Multiply2 x y) = show1 (Multiply1 x y)
...
passwordAllowed  xs (MinimumLength x) = length xs >= x
passwordAllowed xs (ContainsSome ys) = not $ null $ filter (`elem` ys) xs
passwordAllowed xs (DoesNotContain ys) = null $ filter (`elem` ys) xs
passwordAllowed xs (And a b) = passwordAllowed xs a && passwordAllowed xs b
passwordAllowed xs (Or a b ) = passwordAllowed xs a || passwordAllowed xs b
...
operation xs x y = Operation xs x y

evaluate :: Arithmetic -> Integer
evaluate (Literal x) = x
evaluate (Operation xs x y)| xs == "+"  = (+) (evaluate x) (evaluate y)
                            | xs == "*"  = (*) (evaluate x) (evaluate y)

render :: Arithmetic -> String
render (Literal x)         = show x
render (Operation xs x y) = "(" ++ render x ++ xs ++ render y ++ ")"