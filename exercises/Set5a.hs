...
totalPrice (MkShoppingEntry xs x y)= x * fromIntegral y
...
buyOneMore (MkShoppingEntry xs x y) = MkShoppingEntry xs x (y+1)
...
data Person = Persond Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = Persond 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (Persond x xs) = xs

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (Persond x xs) = x

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (Persond x xs) = Persond x name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Persond x xs)= Persond age xs
...
-- up increases the y value of a posit(ion by one
...
study (NthYear x) | x < 7 = NthYear (x+1)
                  | otherwise = Graduated
...
rgb (Invert x) = zipWith (-) [1,1,1] (rgb x)
...
toList Empty= []
toList (Pair x y abs) = [(x,y)] ++ toList abs

fromList :: [(k,v)] -> KeyVals k v
fromList ((x,y): abs)= Pair x y (fromList abs)
fromList [] = Empty
...
toNat x | x <0 = Nothing
        ...
        | x > 1 = Just (PlusOne (fromJust (toNat (x-1))))
