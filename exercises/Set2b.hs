...
oddFactorial x = product (filter odd [1..x])
...
leftpad xs n | length xs < n  = concat (replicate (n-length xs) " ") ++ xs
             | otherwise = xs
...
count :: Show a => a -> String
count n = show n ++ "..."

countdown :: Integer -> String
countdown n = "Ready!" ++ " " ++ unwords (map count (reverse [1..n] )) ++ " " ++ "Liftoff!"
