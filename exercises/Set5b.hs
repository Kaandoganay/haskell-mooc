...
cull val (Node x l r) | x /= val = Node x (cull val l) (cull val r)
                      | otherwise = Empty
...
walk (y:ys) (Node x l r) | y == StepL = walk ys l
                         | otherwise = walk ys r
...
set (y:ys) val (Node x l r) | y == StepL = Node x (set ys val l) r 
                            | otherwise = Node x l (set ys val r)
